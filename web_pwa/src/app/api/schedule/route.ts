...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!weekStartDate || !weekEndDate) {
      return NextResponse.json(
        { success: false, error: "weekStartDate and weekEndDate parameters are required" },
        { status: 400 }
      );
    }

    const erpUrl = `https://kiet.cybervidya.net/api/student/schedule/class?weekStartDate=${weekStartDate}&weekEndDate=${weekEndDate}`;
